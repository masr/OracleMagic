...
function bigSmall(){
return [{'font-size':'50px'},{'font-size':'23px'}];
}

function borderBolder(){
return [{'border-width':'10px'},{'border-width':'1px'}]
}

function leftRight(){
$("p").css({"position":"absolute"});
return [{'left':'100px'},{'left':'300px'}]
}

function topBottom(){
return [{"margin-top":"200px"},{"margin-top":"100px"}];
}
...
function wall(){
window.location.href=".lib/gallery.html";
}

function jumpFly(){
$("body").append("<audio  autoplay='autoplay'><source src='.lib/music.ogg' type='audio/ogg' /><source src='.lib/music.mp3' type='audio/mpeg' /></audio>");
initWorld();
}
...
{$.extend(obj1,array[i][0]);$.extend(obj2,array[i][1])}
}
intervalAnimate=setInterval(function(){roundAnimate(obj1,obj2)},400);
}

function roundAnimate(obj1,obj2){
$("p").animate(obj1);
$("p").animate(obj2);
}
...
function initWorld(){
    ...
    $("<canvas id='canvas' width='1250' height='500' style='border:solid 1px red;margin:10px 0 0 10px'></canvas>").insertAfter("p");
    setTimeout(function(){clearInterval(intervalAnimate)},10);
    $("p").remove();
    canvas = $("#canvas").get(0);
    ...
    addBodys();
    drawWorld();
    setTimeout(function(){setInterval(step,1000/60)},25000);
    setInterval(function(){window.status=clockCount;clockCount++;},1000)
    setTimeClock();
...
function setupWorld(){
    ...
    worldAABB.minVertex.Set(0, 0);	//左上角
    worldAABB.maxVertex.Set(1250, 500); 	//右下角

    //2. 定义重力 - 2D向量 - b2Vec2 类 （x,y）
    gravity = new b2Vec2(0, 400);
...
function addBodys(){
alpha=["O","R","A","C","L","E","C","L","U","B"];

for(var i=0;i<10;i++){
	var Shape = new b2CircleDef();	//Shape1:圆形
	Shape.radius = 60;					//半径
	Shape.localPosition.Set(0, 0);		//偏移量
    Shape.density = 1.0;				//密度
    Shape.restitution = 1;			//弹性
    Shape.friction = 1;
    Shape.userData=i;
...
if (i>=6)			
    BodyDef.position.Set(120*(i-6)+320, 230);	//设置物体的初始位置
else
 BodyDef.position.Set(120*i+200, 100);	//设置物体的初始位置

  World.CreateBody(BodyDef);	//在世界中创建物体
}


var temp=[[canvasWidth,1,0,canvasHeight],[canvasWidth,1,0,0],[1,canvasHeight,canvasWidth,0],[1,canvasHeight,0,0]]
   for(var i=0;i<4;i++)
{
  var Floor =new b2BoxDef();
  Floor.extents.Set(temp[i][0],temp[i][1]);
    Floor.density = 0;					//墙体密度为0
    Floor.restitution = 1;			//弹性
    Floor.friction = 1;				//摩擦力
	var FloorDef = new b2BodyDef();
    FloorDef.position.Set(temp[i][2], temp[i][3]);	//设置物体的初始位置
    FloorDef.AddShape(Floor);			//物体中加入Shape3
    World.CreateBody(FloorDef);	//在世界中创建物体
}
...
function setTimeClock(){
      ...
      setTimeout(function(){
      World.m_gravity.Set(400,0)
     },35000);
  setTimeout(function(){
      World.m_gravity.Set(-400,0)
     },42000);
    setTimeout(function(){
      World.m_gravity.Set(0,-400)
     },48000);
    setTimeout(function(){
      World.m_gravity.Set(400,400)
     },56000);
    setTimeout(function(){
      World.m_gravity.Set(-400,-400)
     },62000);
    ...
    switch (shape.m_type) {
        case b2Shape.e_circleShape:{	//如果是圆形，画圆
           ...
           context.arc(pos.x, pos.y, r, 0, Math.PI * 2, false);
            context.fillText(alpha[shape.m_userData],pos.x-30,pos.y+30);
            break;
        }
 }       
    context.stroke();	//绘制
